...
for x in xrange(1,21):
    sql3=("update atlas_dr3.elais_q0 set nb_real="+str(n_radio-c_radius[x])+" where radius="+str(float(x)/2)+";")
    print sql3,"\n"
    db.query(sql3)
    db.commit()
...
for x in xrange(1,21):
    sql4=("update atlas_dr3.elais_q0 set nr_real="+str(c_radius[x])+" where radius="+str(float(x)/2)+";")
    print sql4,"\n"
    db.query(sql4)
    db.commit()
