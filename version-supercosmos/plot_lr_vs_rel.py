...
# select from matches the sum of L_i grouped by radio source
    ...
    db.query("select lr,reliability from "+schema+"."+schema+"_matches where reliability > 0.0 and reliability < 1.0;" )
...
# Now plot Rel vs LR
    ...
    plot_title='NVSS ' +schema+ ' Reliability vs Likelihood Ratio'
#    plt.title(schema)
...
#    plt.grid(True)
    plt.ylim((-0.1,1.1))
    plot_fname='nvss_' +schema+ '_rel_vs_lr.eps'
...
#    center = (bins[:-1]+bins[1:])/2
    ...
    plot_title='NVSS ' +schema+ ' Histogram of the Reliability values'
#    plt.title(schema)
    plt.yscale('log')
# no grid for publication
#   plt.grid(True)
...
#    pylab.hist(hist, bins=bins, normed=1,histtype='step')
    plot_fname='NVSS_' +schema+ '_N_vs_rel.eps'
    fname=output_dir + plot_fname
    plt.savefig(fname,orientation='landscape',format="eps")
    plt.show()
...
# For elais range=[0.0,100.0] , cdfs range=[0.0,1000.0]
    ...
    print bins,'\n'
    print hist,'\n'
    plot_title='NVSS ' +schema+ ' Histogram of the Likelihood Ratio values'
#    plt.title(schema)
    plt.yscale('log')
    plt.xscale('log')
    plt.ylabel('N(counterparts)')
...
#    plt.plot(center, hist, 'k--', linewidth=1.5)
    plot_fname='atlas_' +schema+ '_N_vs_lr.eps'
    fname=output_dir + plot_fname
    plt.savefig(fname,orientation='landscape',format="eps")
    plt.show()
