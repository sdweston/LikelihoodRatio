...
# We now have two methods, so based on mdbs
    ...
    sql1=("select flux FROM "+schema+"."+schema+"_matches where flux > 0;")

# Use Supercosmos Gama12

    sql1a=("select t2.B_J "
           "FROM "+background_field+".supercosmos_gama12 as t2, "+foreground_field+".nvss_gama12 as t1 "
           "WHERE t2.B_J > 0 "
           "and   pow((t1.ra_2000-t2.RA)*cos(radians(t1.decl_2000)),2)+ "
           "      pow(t1.decl_2000-t2.Decl,2) <= pow("+str(sr)+"/3600,2) "
           "limit 0,20000000;")
...
#this to connect() as the conv keyword parameter.
    ...
    for row in lst_rows:
         a=map(float,row)
#         print "%.4f" % a[0]
#        SuperCosmos B_J is already a optical photometric magnitude, don't need to log it !
#         b=math.log10(a[0])
#    print "%.4f" % b
         f_rows.append(a[0])
...
#    plt.xlim(0,4)
    ...
    plot_title=foreground_field+' total(m)'
    plt.title(plot_title)
    plt.ylabel('total(f)')
    plt.xlabel('B_J')
    plot_fname='atlas_'+foreground_field+'_totalm_vs_log10f.eps'
...
#   The total(m) needs to be a density function per unit arcsec^2
        ...
        log10_f=bins[item]
        db.query("update "+schema+"."+schema+"_n_m_lookup set total_m='%f' \
                  where i='%d';" % (total_m, i))
        db.commit()
        i=i+1
