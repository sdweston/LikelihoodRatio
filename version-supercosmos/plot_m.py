...
# select from matches the sum of L_i grouped by radio source
    ...
    db.query("SELECT i,log10_f,total_m,real_m,n_m FROM %s.%s_n_m_lookup;" % (schema,schema))
...
# Now plot the data
    ...
    plot_title=schema+'  Log10(f) vs #(m)' 
    ...
    plot_fname='atlas_'+schema+'_magnitude_dependance_point.eps' 
...
#    plt.plot(x,y1,linestyle='_',color='k')
    ...
    plt.ylabel('total(m)')
    plt.xlabel('log10(f)')
    plot_fname='atlas_'+schema+'_magnitude_dependance_bar.eps' 
    fname=output_dir + plot_fname
    plt.savefig(fname,format="eps")
    plt.show()
