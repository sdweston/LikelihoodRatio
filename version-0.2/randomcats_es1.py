...
db.query("select count(*),max(ra_12),min(ra_12),max(dec_12),min(dec_12) from fusion.es1;")
...
for x in range(0,source_count):
         ...
         f.write(out_str)
         db.query("insert into elais_s1.es1_randomcat(id,ra,declination) values ('%s','%f','%f');" % (id_str,fake_ra,fake_dec))
         commit=commit+1
         if commit==1000:
             commit=0
             db.commit()
