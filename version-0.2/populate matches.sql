...
truncate table ecdfs.matches;

set session wait_timeout=30000;
set session interactive_timeout=30000;

# we want r in arc-seconds not decimal degrees
insert into ecdfs.matches(elais_s1_cid,swire_index_spitzer,dx,dy,r_arcsec,flux)
select t1.cid,
       t2.Index_Spitzer,
	   (t1.RA_Deg-t2.RA_SPITZER)*cos(t1.Dec_Deg),
	   t1.Dec_Deg-t2.DEC_SPITZER,
...
from ecdfs.coords as t1, swire_cdfs.es1_swire as t2
