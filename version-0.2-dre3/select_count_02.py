# Find fusion sources within x" of a atlas source
...
#    print "Row : ",row
    ...
    ra=float(row[1])
    dec=float(row[2])
    cur.execute("select count(id_12) from fusion.es1 as t2 where pow((%f-t2.RA_12)*cos(%f),2)+ pow(%f-t2.DEC_12,2) <= pow(70/3600,2);" % (ra,dec,dec))
    result=cur.fetchone()
