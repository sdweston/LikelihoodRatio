...
#   So now find all records for the pair of radio source (cid)
    ...
    db.query("SELECT t1.ecdfs_cid,t2.index_spitzer, t2.ra_spitzer, t2.dec_spitzer, \
             format(t1.lr,4), format(t1.reliability,4) \
             FROM ecdfs.matches t1, swire_cdfs.cdfs_swire t2 \
             where t1.lr is not null \
             and (t1.ecdfs_cid='%s' or t1.ecdfs_cid='%s') \
...
#   Define start coords for full txt string for object
    ...
    postage_stamp_filename1='d:\\ecdfs\\150_arcsec\\atlas_'+cid1+'_'+cid2+'.jpeg'
...
#    print cmd1
    ...
    postage_stamp_filename='d:\\ecdfs\\150_arcsec\\swire_'+cid1+'_'+cid2+'_'+ra_radio1+'_'+dec_radio1+'.jpeg'
