...
f=open('RandomCats_cdfs.csv','w')

# Truncate the random catalogue table
db.query("truncate atlas_dr3.cdfs_randomcat;")
...
for x in range(0,source_count):
         ...
         f.write(out_str)
         db.query("insert into atlas_dr3.cdfs_randomcat(id,ra,declination) values ('%s','%f','%f');" % (id_str,fake_ra,fake_dec))
         commit=commit+1
         if commit==1000:
             commit=0
             db.commit()
